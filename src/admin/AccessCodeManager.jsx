    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-end sm:items-center justify-center z-40 p-0 sm:p-4">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-5xl flex flex-col" style={{ maxHeight: '90vh' }}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button onClick={loadCodes} className="text-xs text-gray-500 border border-gray-200 rounded-lg px-3 py-1.5 hover:bg-gray-50 transition-colors">
              ↻ Refresh
            </button>
            <button onClick={() => setShowForm(true)} className="text-xs bg-blue-600 hover:bg-blue-700 text-white font-semibold px-3 py-1.5 rounded-lg transition-colors">
              + New Code
            </button>
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[700px]">
                ...
                <tbody>
                  {filtered.map(row => (
                    <CodeRow key={row.id} row={row} onToggle={handleToggle} onDelete={handleDelete} />
                  ))}
                </tbody>
              </table>
            </div>
        ...
        <div className="px-6 py-3 border-t border-gray-100 flex-shrink-0 flex items-center justify-between">
          <p className="text-xs text-gray-400">Codes are stored in Supabase · No client data is ever synced to the cloud</p>
