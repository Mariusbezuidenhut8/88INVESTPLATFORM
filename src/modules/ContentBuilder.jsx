...
function ParaChip({ para, onAppend, onReplace }) {
  return (
    <div className="group rounded-lg border border-gray-200 bg-white hover:border-blue-200 hover:bg-blue-50/50 p-3 mb-2 transition-all">
...
function SectionEditor({ section, productKey, text, onChange, clientProfile, providerResult }) {
  ...
  const filtered   = filter ? paragraphs.filter(p => p.label.toLowerCase().includes(filter.toLowerCase()) || p.tags?.some(t => t.includes(filter.toLowerCase()))) : paragraphs;
      ...
      {showBank && (
        <div className="w-72 flex-shrink-0 border-r border-gray-200 bg-gray-50 flex flex-col" style={{maxHeight:'400px'}}>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-2">
            ...
            {filtered.map(p => <ParaChip key={p.id} para={p} onAppend={handleAppend} onReplace={handleReplace} />)}
          </div>
        </div>
      )}
